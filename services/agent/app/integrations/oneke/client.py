...
NEWS_ENTITY_TYPES = DEFAULT_ENTITY_TYPES
NEWS_RELATION_TYPES = [
    (r["subject"], r["relation"], r["object"])
    for r in DEFAULT_RELATION_TYPES
]
...
class OneKEClient:
    ...
    def _resolve_schema(self, schema_name: str | None) -> tuple[list[str], list[tuple[str, str, str]], str | None]:
        """Resolve schema configuration from registry or fall back to defaults.

        Returns (entity_types, relation_types, instruction).
        """
        name = schema_name or "MOE_News"
        if self.schema_registry is not None:
            schema = self.schema_registry.get_by_name(name)
            if schema is not None:
                relations = [
                    (r["subject"], r["relation"], r["object"])
                    for r in schema.relation_types
                ]
                return schema.entity_types, relations, schema.instruction
        # Fall back to defaults for backward compatibility
        return DEFAULT_ENTITY_TYPES, DEFAULT_RELATION_TYPES, None
    ...
    def _build_remote_payload(self, *, text: str, schema_name: str | None) -> dict[str, Any]:
        ...
        entity_types, relation_types, instruction = self._resolve_schema(schema_name)
        entity_types_str = ", ".join(entity_types)
        relation_examples = "; ".join([f"{s}-{r}->{t}" for s, r, t in relation_types[:5]])
